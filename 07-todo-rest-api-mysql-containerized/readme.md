...
docker run --detach --env MYSQL_DATABASE=todos --env MYSQL_ROOT_PASSWORD=dummypassword --env MYSQL_USER=todos-user --env MYSQL_PASSWORD=dummytodos --name mysql --publish 3306:3306 mysql:5.7```
```
...
```
mvn clean package
docker run --name todos-api --publish 5000:5000 --link mysql:mysql tesahu/todo-rest-api-mysql:1.0.0.RELEASE
```
